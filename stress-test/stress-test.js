 ...
 *
 *  Phase 1 — Ramp-Up    : 0 → 5,000 VUs over 3 minutes
 *  Phase 2 — Sustained  : hold 5,000 VUs for 5 minutes
 *  Phase 3 — Ramp-Down  : 5,000 → 0 VUs over 2 minutes
 *
...
export default function() {
  ...
  if (ok) {
    ...
    if (__ENV.VERBOSE === "true") {
      console.error(
        `FAIL | status=${res.status} | duration=${res.timings.duration}ms | body=${res.body?.slice(0, 200)}`,
      );
    }
  }
